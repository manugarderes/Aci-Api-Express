...
export const getMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const tasaRecuperacion = tickets.length > 0 
    ...
    const hoy = new Date();
    const ticketsVencidos = tickets.filter(t => !t.paid && new Date(t.due_date) < hoy);
    
    const diasMoraPromedio = ticketsVencidos.length > 0
      ? ticketsVencidos.reduce((sum, t) => {
          const diferenciaBus = hoy.getTime() - new Date(t.due_date).getTime();
